...
router.post("/add", ensureAuthenticated, (req, res) => {
    ...
    HackingProduct.findOne({where: {title}}).then( hackingProduct => { // Find if the product is in the db
        CartItem.findOne({where: {title, userId:req.user.id}}).then(cartItem => {
            ...
            if(cartItem){ 
                ...
                CartItem.update(updateCartEntry, {where: {id: cartItem.id}}).then(result => {
                    res.json("Updated!")
                })
            } else {
                ...
                CartItem.create(cartEntry).then(result => {
...
router.post("/removeItem", ensureAuthenticated, (req, res)=>{
    ...
    CartItem.findOne({where:{title, userId: req.user.id}}).then(cartItem =>{
        if(cartItem){
            CartItem.destroy({where:{title, userId: req.user.id}}).then(result => {
...
router.post("/valueUpdate", (req, res) => {
    ...
    CartItem.findOne({where:{title, userId: req.user.id}}).then(cartItem => {
        if(cartItem){
            CartItem.update(updateNumEntry, {where: {id: cartItem.id}}).then(result => {
                    res.json("Updated!");
                })
        } else{
